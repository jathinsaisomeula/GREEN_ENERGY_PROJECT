...
country_profiles = {
    'India': {
        ...
        'policy_base': 3, 'policy_growth': 0.2, 'policy_count_initial': 5, # Added for Policy_Count
    ...
    'Australia': {
        ...
        'policy_base': 6, 'policy_growth': 0.1, 'policy_count_initial': 10,
    ...
    'Germany': {
        ...
        'policy_base': 8, 'policy_growth': 0.05, 'policy_count_initial': 20,
    ...
    'USA': {
        ...
        'policy_base': 5, 'policy_growth': 0.12, 'policy_count_initial': 15,
    ...
    'UK': {
        ...
        'policy_base': 7, 'policy_growth': 0.08, 'policy_count_initial': 18,
    ...
    'Canada': {
        ...
        'policy_base': 6.5, 'policy_growth': 0.09, 'policy_count_initial': 12,
...
for country in countries:
    ...
    # Initialize annual policy count for this country
    current_annual_policy_count = profile['policy_count_initial']
    ...
    for date in dates:
        ...
        # --- Existing Metrics (some calculations adjusted for new columns) ---
        ...
        row['CO2_Emissions_Million_Tons'] = profile['co2_base'] * co2_modifier + np.random.normal(0, 0.5)
        
        # NEW: Carbon_Footprint_Per_Capita_Tons (conceptual differentiation)
        # This will be based on CO2 but potentially includes a wider scope or different scaling
        row['Carbon_Footprint_Per_Capita_Tons'] = (row['CO2_Emissions_Million_Tons'] * 1000000 / (row['Population_Millions'] * 1000000)) * 1.1 # Slightly higher or scaled to represent broader footprint
        row['Air_Quality_Index_Avg'] = max(20, 100 - (year - start_year) * 2 + np.random.normal(0, 3) + profile['energy_consumption_seasonality'][month-1] * 5)
        ...
        row['Hydro_Generation_TWh'] = (renewable_total_twh_annual * (0.2 + np.random.uniform(-0.05, 0.05)) / 12) # Simplified for monthly, assume consistent small portion
        row['Other_Renewable_Generation_TWh'] = (renewable_total_twh_annual * 0.1 / 12) + np.random.normal(0, 0.1)
        ...
        row['Fossil_Fuel_Generation_TWh'] = (fossil_fuel_total_twh_annual / 12) + np.random.normal(0, 0.5) # This is our Fossil_Fuel_Usage
        ...
        # NEW: Policy_Count_Last_5_Yrs (simulated proxy)
        # Update annual policy count once per year
        if month == 1: # Only update annually
             current_annual_policy_count = max(profile['policy_count_initial'], profile['policy_count_initial'] + (year - start_year) * np.random.choice([0.5, 0.7, 1.0, 1.2])) # Increment annually
        row['Policy_Count_Last_5_Yrs'] = int(min(50, current_annual_policy_count + np.random.normal(0, 1))) # Add some monthly noise, but keep based on annual trend
        ...
        row['Average_Electricity_Price_USD_per_kWh'] = max(0.08, 0.15 - (year - start_year) * 0.003 + np.random.normal(0, 0.005) + profile['energy_consumption_seasonality'][month-1] * 0.01)
        ...
        row['Public_Green_Awareness_Score'] = min(10, 5 + (row['Green_Policy_Strength_Index'] / 2) + (row['Green_Marketing_Spend_Per_Capita_USD'] * 10) + np.random.normal(0, 0.2))
        
        # NEW: Sustainability_Score (normalized index)
        # Influenced by CO2, RE Share, Recycling, Policy Strength
        sustainability_components = [
            (1 - (row['CO2_Emissions_Million_Tons'] / (profile['co2_base'] * 1.5))) * 0.3, # Inverse CO2, capped at reasonable max
            (row['Renewable_Energy_Share_Percentage'] / 100) * 0.3,
            (row['Waste_Recycling_Rate_Percentage'] / 100) * 0.2,
            (row['Green_Policy_Strength_Index'] / 10) * 0.2 # Scale 0-10
        ]
        row['Sustainability_Score'] = min(100, max(0, sum(c for c in sustainability_components if c is not None) * 100 + np.random.normal(0, 2))) # Scale to 0-100 and add noise
...
for col in ['Green_Policy_Strength_Index', 'Air_Quality_Index_Avg', 'EV_Sales_Percentage_of_New_Cars', 'Waste_Recycling_Rate_Percentage', 'Sustainability_Score']:
...
for col in df_green_energy_monthly.columns:
    if col not in ['Country', 'Year', 'Month', 'Date']:
        ...
        if 'Percentage' in col or 'Index' in col or 'Price' in col or 'Per_Capita' in col or 'Share' in col or 'Score' in col:
...
print("Generated Monthly Green Energy Dataset Head (with new columns):")
print(df_green_energy_monthly.head())
print("\nGenerated Monthly Green Energy Dataset Info (with new columns):")
...
df_green_energy_monthly.to_csv('custom_green_energy_monthly_data_v2.csv', index=False)
print("\nDataset saved to 'custom_green_energy_monthly_data_v2.csv'")